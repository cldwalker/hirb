...
Hirb provides a mini view framework for console applications, designed to improve irb's default inspect output.
Given an object or array of objects, hirb renders a view based on the object's class and/or ancestry. Hirb bundles reusable
...
for generating ascii tables and trees. Using these helpers, hirb has useful default views for popular database classes i.e. Rails'
ActiveRecord::Base (see Hirb::Helpers::AutoTable). Other than views, hirb also offers a smart pager and a menu widget. The smart pager
only pages when the output exceeds the current screen size. The menu widget is used in conjunction with tables to offer
...
If a class isn't configured, Hirb reverts to irb's default echo mode.
  >> Hirb::Formatter.default_config['ActiveRecord::Base']
  ...
  >> Hirb::Formatter.default_config['String']
  => nil
  >> 'plain ol irb'
  => 'plain ol irb'
  >> Hirb::Formatter.default_config['Symbol']
  => nil
  >> :blah
  => :blah
...
* Provide helper methods to all view classes.

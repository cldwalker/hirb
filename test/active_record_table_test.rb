...
class Hirb::Helpers::ActiveRecordTableTest < Test::Unit::TestCase
  context "activerecord table" do
    test "with no select gets default options" do
      ...
      Hirb::Helpers::AutoTable.active_record__base_options(pet).should == {:fields=>[:age, :name]}
    end
    ...
    test "with select gets default options" do
      ...
      Hirb::Helpers::AutoTable.active_record__base_options(pet).should == {:fields=>[:name]}
    end
  end
end